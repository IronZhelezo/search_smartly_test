...
const INITIAL_TABLE_SIZE = 13;
const POSS = ['Up', 'Right', 'Down', 'Left'];
const INTERVAL = 1000;
const DEFAULT_ACTIVE = 'y:0-x:0';

const getFields = (size = INITIAL_TABLE_SIZE) => {
  let defaultArray = new Array(size);
  const arrLength = defaultArray.length;
  const half = Math.floor(arrLength / 2); 
  defaultArray.fill(0, 0, arrLength);
  return defaultArray.reduce((acc, d, i) => {
    acc[i] = defaultArray.map((n, j) => ({ y: i - half, x: j - half, id: `y:${i - half}-x:${j - half}` }));
    return acc;
  }, []);
};
...
const Cell = ({
  ...
  <div
    key={id}
    id={id}
    className={`${cell} ${isActive && cellActive} ${isBlack(id) && cellBlack}`}
  >
    {isActive && (
      <svg
        version="1.0"
...
const getNewActive = (arr, pos, prevActive) => {
  let { x, y } = prevActive;
  switch (pos) {
    case 'Down': y += 1; break;
...
const LangtonAnt = () => {
  ...
  const isBlack = (id) => !!blackCells.find((item) => item === id);
  useEffect(() => {
    if (ref && ref.current) {
  ...
  useEffect(() => {
    if (!!(step % 2)) {
      const activeIsBlack = blackCells.find((i) => i === active);
      setPos((p) => {
        const index = POSS.findIndex((d) => d === p);
        const stepLeft = POSS[index - 1] ? POSS[index - 1] : POSS[POSS.length - 1];
        const stepRight = POSS[index + 1] ? POSS[index + 1] : POSS[0];
        return activeIsBlack ? stepLeft : stepRight;
      });
      setBlackCells((arr) => {
        if (!activeIsBlack) {
          arr.push(active);
      ...
      fields.forEach((d) => {
          if (!prevActive) {
            prevActive = d.find(({ id }) => id === active);
          }
      });
      if (prevActive) {
        let newActive = getNewActive(fields, position, prevActive);
        if (!newActive) {
          const newSize = size * 2;
          setSize(newSize);
          const newFields = getFields(newSize);
          setFields(newFields);
          newActive = getNewActive(newFields, position, prevActive)
        }
        setActive(newActive);
      }
    }
  ...
  return (
    <div
      className={`${ant} ${step > 410 && antMiddleScaled} ${step > 1800 && antVeryScaled}`}
      ref={ref}
    >
      {fields.map((item) => (
        <div
